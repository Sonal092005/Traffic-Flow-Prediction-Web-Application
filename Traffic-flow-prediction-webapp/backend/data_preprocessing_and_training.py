...
DATA_PATH = os.path.join(PROJECT_ROOT, "data", "bangalore_traffic_dataset_lstm.csv")
...
data = data[
    ...
    (data["junction"] >= 0) & (data["junction"] <= 8) &
...
scaler_y = MinMaxScaler(feature_range=(0, 1))  # model output ~ [0, 1]
...
class TrafficLSTM(nn.Module):
    ...
    def forward(self, x):
        _, (hn, _) = self.lstm(x)
        out = self.fc(hn[-1])
        # No clamp: let model learn freely in scaled space
        return out
...
torch.save(model.state_dict(), os.path.join(MODEL_DIR, "traffic_lstm.pth"))

print("✅ Training completed successfully.")
print("✅ Model trained in scaled [0,1] space; use scaler_y.inverse_transform at inference.")
